...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if(date("H",time())<16 || (date("H",time())==16 && date("i",time()) <=15)){
    ...
    public function actionXosomiennam()
    {
        $this->render("miennam");
    }
    ...
    public function actionXosomienbac()
    {
        $this->render("mienbac");
    }
    
    public function actionXosotinh($id)
    {
        if($id<=20 || $id==41) $model='KqxsNam';
        else if($id>=21 && $id<=26) $model='KqxsBac';
        else $model='KqxsBac';
        $criterianam = new CDbCriteria();
        $criterianam->condition='provice=:provice';
        $criterianam->params=array(':provice'=>$id);
        $criterianam->limit = 5;
        $criterianam->order = "id desc";
        $bac = $model::model()->findAll($criterianam);

        $this->render("xosotinh",array('bac'=>$bac,'id'=>$id));
    }
    
    public function actionXosongay($ngay)
    {
        if (preg_match('/^(0[1-9]|[1-2][0-9]|3[0-1])-(0[1-9]|1[0-2])-[0-9]{4}$/', $ngay))
        {
            $exlodethu = explode("-",$ngay);
            $time = mktime(12,12,12,$exlodethu[1],$exlodethu[0],$exlodethu[2]);
            $day = date("l",$time);
            $data = Calendar::model()->find("thu = :thu", array(":thu"=>$day));
            $nam = explode(",",$data->nam);
            $trung = explode(",",$data->trung);
            ...
            $criterianam = new CDbCriteria();
            $criterianam->condition='date=:date';
            $criterianam->params=array(':date'=>$ngay);
            $criterianam->addInCondition("provice", $nam);
            $resultnam = KqxsNam::model()->findAll($criterianam);
            ...
            $criteriatrung = new CDbCriteria();
            $criteriatrung->condition='date=:date'; 
            $criteriatrung->params=array(':date'=>$ngay);
            $criteriatrung->addInCondition("provice", $trung);
            $resulttrung = KqxsTrung::model()->findAll($criteriatrung);
            ...
            $criteriabac = new CDbCriteria();
            $criteriabac->condition='date=:date'; 
            $criteriabac->params=array(':date'=>$ngay);
            $criteriabac->addInCondition("provice", $bac);
            $resultbac = KqxsBac::model()->findAll($criteriabac);
            ...
            $this->render('xosongay', array(
             ...
             throw new CHttpException(201,'Ngày dò không hợp lệ !');
        }
    }
    
    public function actionOnlinenam()
    {
        $day = date("l",time());
        $date = date("d-m-Y",time());
        ...
        $criterianam->addInCondition("provice", $nam);
        $resultnam = KqxsNam::model()->findAll($criterianam);
        $this->render("onlinemiennam",array('nam'=>$resultnam));
    }
    public function actionOnlinetrung()
    {
        $day = date("l",time());
        $date = date("d-m-Y",time());
        ...
        $resultnam = KqxsTrung::model()->findAll($criterianam);
        $this->render("onlinemientrung",array('nam'=>$resultnam));
    }
