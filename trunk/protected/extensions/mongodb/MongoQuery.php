 ...
 *  $result = MongoQuery::instance('members')->findArray($criteria,$select,$sort,$limit);
 * </pre>
 *
...
class MongoQuery
{
	...
	/**
	 * Create a MongoQuery for the specified collection
	 * Uses the preconfigured connection from MongoBehavior if dbName, $connectionId is not set
	 *
     ...
     * called after creating the MongoCollection instance
     *
     * Example: MongoQuery::instance($collectionName)->setCallAfterCursorCreate(array('limit'=>1,'sort'=>array('weight'=>1,'title'=>-1)))->findArray($criteria,$select);
     *
     * @param array $methods
     * @return MongoQuery
     * @since 0.25
	...
	{
		if (!isset($this->_db))
		  $this->_db = Yii::app()->mongoDb($this->_dbName,$this->_connectionId);
	...
	public function getCollection()
	{
        ...
        {
            $this->_collection = Yii::app()->mongoCollection($this->_collectionName,$this->_dbName,$this->_connectionId);
	 ...
	 *
	 * Usage:
	 * $criteria = MongoQuery::getWordsSearchCriteria('example for',array('title','subtitle'));
	 ...
	 * Uses the preconfigured connection from the MongoBehavior
	 *
	...
	*/
	public function getArrayDataProvider($criteria=array(),$select=array(),$config=array())
	{
		...
		return new MongoDataProvider($this->_cursor,$config);
	}
	...
	{
        $this->_cursor = $this->findCursor($criteria,$select);
		return new MongoDataProvider($this->_cursor,$config,'stdClass');
	}
	...
	{
        $this->_cursor = $this->findCursor($criteria,$select);
		return new MongoDataProvider($this->_cursor,$config,$modelClassname);
	}

	/**
	 * Uses the preconfigured connection from the MongoBehavior to find one record
	 *
