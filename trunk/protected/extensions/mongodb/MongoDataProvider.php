...
/**
 * Create a DataProvider from a {@link http://www.php.net/manual/en/class.mongocursor.php MongoCursor}.
 * Support for sort and pagination.
 *
 * <h2>Configuration</h2>
 * <pre class="prettyprint">
 *  $cursor = $collection->find(); //MongoCursor
 *  $dataProvider = new MongoDataProvider($cursor,
 *                                      array(
...
class MongoDataProvider extends CDataProvider {
    ...
    protected function fetchData() {
        ...
        if (isset($this->_objectClassName) && $this->_objectClassName != 'stdClass')
        {
            ...
            if (!$modelInstance instanceof CModel) { //CModel uses setAttributes in MongoQuery::arrayToModel
    ...
    protected function fetchKeys()
    {
        ...
        foreach ($this->getData() as $i => $data)
            if (is_array($data)) $keys[$i] = $data['_id'];
        return $keys;
    }
